...
class EgyBest:
    ...
    def get_content_url(self):
        ...
        for i, link in enumerate(res, 1):
            ...
            try:
                ...
                ch+=str(i)+"-> "+ link.contents[4].text+" , Type : "+content_type+", "+" IMDB Rating : "+link.contents[0].i.i.text+"\n"
            except:
                ch+=str(i)+"-> "+ link.contents[2].text+" , Type : "+content_type+"\n"
        print(ch)
        while res:
            try:
    ...
    def start_download(self, url, option: "Get download link 0,Append to IDM 1" = 1) -> "Returns the url":
        global selected_quality
        self.chrome_driver.get(url)
        print("Accessing EgyBest page")
        if selected_quality==0:
            qt=self.chrome_driver.find_elements_by_xpath('''//table[@class="dls_table btns full mgb"]//tbody//tr''')
            for i,c in enumerate(qt,1):
                print(i,"-->"," - ".join(list(map(lambda x: x.text, c.find_elements_by_tag_name("td")))[:-1]))
            i=self.get_int_input("select quality :")
            selected_quality=i
        else: i=selected_quality
        target_button = self.chrome_driver.find_elements_by_class_name('btn.g.dl.nop._open_window')[i-1]
        deviation=self.chrome_driver.find_element_by_xpath("//html/body/div[4]/div[2]/div/div[3]")
        deviation.click()
        ...
        self.chrome_driver.switch_to.window(self.chrome_driver.window_handles[0])
        while 1:
            target_button = self.chrome_driver.find_element_by_xpath("/html/body/div[1]/div/p[2]/a[1]")
            try:
